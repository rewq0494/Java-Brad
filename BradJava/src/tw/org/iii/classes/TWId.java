...
public class TWId {
	...
	public TWId() {
		this((int) (Math.random() * 2) == 0);//第一道敘述句只能用super or this
	}

	public TWId(boolean isMale) { // 性別
		this(isMale, (int) (Math.random() * 26));
	}

	public TWId(int area) { // 區域
		this((int) (Math.random() * 2) == 0, area);
	}

	public TWId(boolean inMale, int area) {
		String c1 = letters.substring(area, area + 1);
		StringBuilder sb = new StringBuilder(c1);
		sb.append(inMale ? "1" : "2");
		for (int i = 0; i < 7; i++) {
